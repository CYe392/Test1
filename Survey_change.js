        ...
        var SurveyServiceNew = (function()
            {
                ...
                debugger;
                	...
                	serverResult.sort(function(a,b){
                		var questionOrderA = FisherJS.Survey.Utils.Query.getAttribute(a, "fi_ordernumber",999);
                		var answerOrderA = FisherJS.Survey.Utils.Query.getAttribute(a, "a.fi_ordernumber",999);
                		var questionOrderB = FisherJS.Survey.Utils.Query.getAttribute(b, "fi_ordernumber",999);
                		var answerOrderB = FisherJS.Survey.Utils.Query.getAttribute(b, "a.fi_ordernumber",999);
                		if(questionOrderA > questionOrderB)
                			return 1;
                		else if(questionOrderA < questionOrderB)
                			return -1;
                		else if(answerOrderA < answerOrderB)
                			return 1;
                		else if(answerOrderA > answerOrderB)
                			return -1;
                		else
                	...
                	var i = 0;
                	while(i<serverResult.length) {
	                    var question = {
	                        fi_answertype: { Value: parseInt(FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "fi_answertype"))},
	                        fi_description: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "fi_description"),
	                        fi_hasseparator: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "fi_hasseparator"),
	                        fi_ordernumber: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "fi_ordernumber"),
	                        fi_questionId: serverResult[i].Id,
	                        fi_questiontext: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "fi_questiontext"),
	                        fi_question_fi_answeroption: { results: new Array() }
	                    };
	                    do {
	                        question.fi_question_fi_answeroption.results.push({
	                            fi_answer: parseInt(FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "a.fi_answer")),
	                            fi_answeroptionId: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "a.fi_answeroptionid"),
	                            fi_istextresponserequired: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "a.fi_istextresponserequired"),
	                            fi_ordernumber: FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "a.fi_ordernumber"),
	                            fi_textresponsetype: { Value: parseInt(FisherJS.Survey.Utils.Query.getAttribute(serverResult[i], "a.fi_afi_textresponsetypenswer"))}
	                        });
	                        i++;
	                    }
	                    while(i<serverResult.length && serverResult[i-1].Id == serverResult[i].Id);
